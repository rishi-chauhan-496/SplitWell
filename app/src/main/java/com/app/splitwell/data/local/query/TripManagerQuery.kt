...
import com.app.splitwell.data.local.database.Database.TripManagerTable
import com.app.splitwell.data.local.database.Database.UserTable
import com.app.splitwell.data.local.model.TripManager
import com.app.splitwell.data.remote.group.Member
...
class TripManagerQuery(private val dbHelper: Database) {
    ...
    // ---------------- RECONCILE (catches members the server leaves out entirely) ----------------
    fun markInactiveIfMissing(tripId: String, activeUserIds: Set<String>) {
        val db = dbHelper.writableDatabase
        val cv = ContentValues()
        cv.put(TripManagerTable.IS_ACTIVE, 0)

        if (activeUserIds.isEmpty()) {
            db.update(
                TripManagerTable.TABLE_NAME, cv,
                "${TripManagerTable.TRIP_ID} = ? AND ${TripManagerTable.IS_ACTIVE} = 1",
                arrayOf(tripId)
            )
            return
        }

        val placeholders = activeUserIds.joinToString(",") { "?" }
        val args = (listOf(tripId) + activeUserIds).toTypedArray()

        db.update(
            TripManagerTable.TABLE_NAME, cv,
            "${TripManagerTable.TRIP_ID} = ? AND ${TripManagerTable.IS_ACTIVE} = 1 " +
                    "AND ${TripManagerTable.USER_ID} NOT IN ($placeholders)",
            args
        )
    }
}